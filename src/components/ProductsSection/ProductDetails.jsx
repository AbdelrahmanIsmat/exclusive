import { Link, useParams } from "react-router-dom";
import { productsData } from "../../productsData"; // عدّل المسار حسب مشروعك

import Button from "../common/Button";
import { useState } from "react";
import { useFavorites } from "../context/FavoritesProvider";
import { useCart } from "../context/CartContext";
import toast, { Toaster } from "react-hot-toast";

const ProductDetails = () => {
  const { id } = useParams(); // ناخد id من الرابط
  const [quantity, setQuantity] = useState(1);
  const { addToFavorites, isFavorite, removeFromFavorites } = useFavorites();
  const { addManyToCart } = useCart();

  const toastSuccess = () =>
    toast.success(
      <div className="flex flex-col items-center gap-3 ">
        {<img className="h-15" src={product.image} alt={product.title} />}
        {<p className="text-2xl">x{quantity}</p>}
        {
          <Link
            to="/CartPage"
            className="text-[13px] rounded-full bg-[#DB4444] text-white px-10 py-3  "
          >
            Go to Cart
          </Link>
        }
      </div>,
      { duration: 3000 }
    );
  const handleAddManyToCart = () => {
    addManyToCart(
      Array(quantity).fill(product) // ننشئ مصفوفة تحتوي على نفس المنتج بعدد الكمية
    );
    setQuantity(1); // إعادة تعيين الكمية إلى 1 بعد الإضافة
    toastSuccess();
  };

  const product = productsData.find((p) => p.id === parseInt(id));
  if (!product) return <p>Product not found</p>;

  return (
    <div className="flex justify-between max-w-6xl mx-auto p-6 gap-10 md:flex-row flex-col">
      {/* Main Image */}
      <div className=" rounded-lg w-full max-w-125 flex items-center justify-center md:items-center p-6 bg-[#F5F5F5]">
        <img
          src={product.image}
          alt={product.title}
          className="flex md:justify-center w-full max-h-100 object-contain"
        />
      </div>
      {/* Info */}
      <div className="flex flex-col  gap-4">
        <h1 className="text-2xl font-semibold">{product.title}</h1>
        {/* Rating */}
        <div className="flex items-center gap-2">
          <img src={product.ratingImage} alt="rating" />
          <span className="text-sm text-gray-500">
            ({product.reviews} Reviews)
          </span>
          <span className="text-green-600 text-sm">In Stock</span>
        </div>

        {/* Price */}
        <p className="text-2xl font-bold">${product.price}</p>

        {/* Description */}
        <p className="text-gray-600 text-sm leading-relaxed">
          {product.description}
        </p>
        <hr />
        {/* Colours */}
        <div className="flex items-center gap-3">
          <span className="font-medium">Colours:</span>
          <span className="w-4 h-4 rounded-full bg-black cursor-pointer" />
          <span className="w-4 h-4 rounded-full bg-red-400 cursor-pointer" />
        </div>

        {/* Size */}
        <div className="flex items-center gap-3">
          <span className="font-medium">Size: </span>
          {["XS", "S", "M", "L", "XL"].map((size) => (
            <button
              key={size}
              className={`border border-gray-500 px-3 py-1 text-sm rounded ${
                size === "M"
                  ? "bg-[#DB4444] text-white border-[#DB4444]"
                  : "hover:border-black"
              }`}
            >
              {size}
            </button>
          ))}
        </div>
        {/* Quantity + Buy */}
        <div className="flex flex-wrap items-center gap-4 mt-4">
          {/* Quantity */}
          <div className="flex items-center max-h-14 ">
            {/* Minus */}
            <button
              onClick={() => setQuantity((prev) => (prev > 1 ? prev - 1 : 1))}
              className="border border-gray-500 rounded-tl rounded-bl px-3 py-4 text-4xl font-extrabold cursor-pointer  hover:bg-[#DB4444]  "
              disabled={quantity === 1}
            >
              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="hover:stroke-white  transition-colors "
              >
                <path
                  d="M20 12H4"
                  stroke="black"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                />
              </svg>
            </button>
            {/* Quantity */}
            <span className="py-4 px-3 border-y border-gray-500 font-semibold text-center">
              {quantity}
            </span>
            {/* Plus */}
            <button
              onClick={() => setQuantity((prev) => prev + 1)}
              className="border border-gray-500 rounded-tr rounded-br px-3 py-4 text-4xl font-extrabold cursor-pointer hover:bg-[#DB4444] hover:text-white transition-colors"
            >
              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M12 20V12M12 12V4M12 12H20M12 12H4"
                  stroke="black"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                />
              </svg>
            </button>
          </div>
          {/* Buy */}
          <Button onClick={handleAddManyToCart} children={"Buy Now"} />
          <Toaster
            position="bottom-right"
            reverseOrder={true}
            toastOptions={{
              style: {
                width: "250px",
              },
            }}
          />
          <div className="">
            <button
              onClick={() => {
                if (isFavorite(product.id)) {
                  removeFromFavorites(product.id);
                } else {
                  addToFavorites(product.id);
                }
              }}
              className="cursor-pointer w-8 h-8 border-gray-500 border rounded flex items-center justify-center shadow transition-transform duration-200 hover:scale-110"
            >
              <svg
                width="20"
                height="20"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className={`transition-all duration-200 ${
                  isFavorite(product.id)
                    ? "fill-[#DB4444] stroke-[#DB4444]"
                    : "fill-none stroke-black"
                }`}
              >
                <path
                  d="M12 21s-7-4.35-9.33-8.17C.78 9.61 2.14 5.5 6.28 5.5 
                  8.62 5.5 10.24 6.91 12 8.5 
                  13.76 6.91 15.38 5.5 17.72 5.5 
                  21.86 5.5 23.22 9.61 21.33 12.83 
                  19 16.65 12 21 12 21Z"
                  stroke="black"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
              </svg>
            </button>
          </div>
        </div>
        {/* Delivery */}
        <div className="border rounded mt-6">
          <div className="p-4 flex gap-3 border-b">
            <svg
              width="40"
              height="40"
              viewBox="0 0 40 40"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clip-path="url(#clip0_261_4843)">
                <path
                  d="M11.6673 31.6667C13.5083 31.6667 15.0007 30.1743 15.0007 28.3333C15.0007 26.4924 13.5083 25 11.6673 25C9.82637 25 8.33398 26.4924 8.33398 28.3333C8.33398 30.1743 9.82637 31.6667 11.6673 31.6667Z"
                  stroke="black"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
                <path
                  d="M28.3333 31.6667C30.1743 31.6667 31.6667 30.1743 31.6667 28.3333C31.6667 26.4924 30.1743 25 28.3333 25C26.4924 25 25 26.4924 25 28.3333C25 30.1743 26.4924 31.6667 28.3333 31.6667Z"
                  stroke="black"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
                <path
                  d="M8.33398 28.3335H7.00065C5.89608 28.3335 5.00065 27.4381 5.00065 26.3335V21.6668M3.33398 8.3335H19.6673C20.7719 8.3335 21.6673 9.22893 21.6673 10.3335V28.3335M15.0007 28.3335H25.0007M31.6673 28.3335H33.0007C34.1052 28.3335 35.0007 27.4381 35.0007 26.3335V18.3335M35.0007 18.3335H21.6673M35.0007 18.3335L30.5833 10.9712C30.2218 10.3688 29.5708 10.0002 28.8683 10.0002H21.6673"
                  stroke="black"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
                <path
                  d="M8 28H6.66667C5.5621 28 4.66667 27.1046 4.66667 26V21.3333M3 8H19.3333C20.4379 8 21.3333 8.89543 21.3333 10V28M15 28H24.6667M32 28H32.6667C33.7712 28 34.6667 27.1046 34.6667 26V18M34.6667 18H21.3333M34.6667 18L30.2493 10.6377C29.8878 10.0353 29.2368 9.66667 28.5343 9.66667H21.3333"
                  stroke="black"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
                <path
                  d="M5 11.8182H11.6667"
                  stroke="black"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
                <path
                  d="M1.81836 15.4545H8.48503"
                  stroke="black"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
                <path
                  d="M5 19.0909H11.6667"
                  stroke="black"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
              </g>
              <defs>
                <clipPath id="clip0_261_4843">
                  <rect width="40" height="40" fill="white" />
                </clipPath>
              </defs>
            </svg>
            <div>
              <p className="font-medium">Free Delivery</p>
              <p className="text-sm text-gray-500">
                Enter your postal code for Delivery Availability
              </p>
            </div>
          </div>
          <div className="p-4 flex gap-3">
            <svg
              width="40"
              height="40"
              viewBox="0 0 40 40"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clip-path="url(#clip0_261_4865)">
                <path
                  d="M33.3327 18.3334C32.9251 15.4004 31.5645 12.6828 29.4604 10.5992C27.3564 8.51557 24.6256 7.18155 21.6888 6.80261C18.752 6.42366 15.7721 7.02082 13.208 8.5021C10.644 9.98337 8.6381 12.2666 7.49935 15M6.66602 8.33335V15H13.3327"
                  stroke="black"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
                <path
                  d="M6.66602 21.6667C7.07361 24.5997 8.43423 27.3173 10.5383 29.4009C12.6423 31.4845 15.3731 32.8185 18.3099 33.1974C21.2467 33.5764 24.2266 32.9792 26.7907 31.4979C29.3547 30.0167 31.3606 27.7335 32.4994 25M33.3327 31.6667V25H26.666"
                  stroke="black"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
              </g>
              <defs>
                <clipPath id="clip0_261_4865">
                  <rect width="40" height="40" fill="white" />
                </clipPath>
              </defs>
            </svg>
            <div>
              <p className="font-medium">Return Delivery</p>
              <p className="text-sm text-gray-500">
                Free 30 Days Delivery Returns. Details
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default ProductDetails;
